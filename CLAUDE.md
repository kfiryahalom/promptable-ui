...
The root is the publishable package itself. `examples/` contains demo apps that consume it via `workspace:*`.

```
src/                        ← package source (root level)
  components/
...
### task-pulse (`examples/task-pulse/`)
...
- Run: `cd examples/task-pulse && pnpm dev`

### fit-board (`examples/fit-board/`)
...
- Run: `cd examples/fit-board && pnpm dev`

### market-lens (`examples/market-lens/`)
...
- Run: `cd examples/market-lens && pnpm dev`
...
Edit `src/components/CustomizeModal.tsx`. All styling is inline styles — no Tailwind. The two-layer structure (edit modal z-50, preview modal z-60) must be preserved.

**Add a new example app:**
Create `examples/<name>/` with its own `package.json` (name it so pnpm workspace picks it up via `examples/*`). Use `LocalStorageAdapter` for quick setup or `ServerAdapter` for persistence.
